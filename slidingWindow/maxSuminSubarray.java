...
class Solution{
    static long maximumSumSubarray(int K, ArrayList<Integer> Arr,int N){
        ...
        int current_sum=0;
        // iterate upto k and do sum
        for(int i=0;i<K;i++){
            current_sum+=Arr.get(i);
        }
        //store that current sum of each window in max
        max=current_sum;
        // after finding sum of one window move the window by removing (i-k)th element and adding i ie end pointer element
        for(int i=K;i<N;i++){
            current_sum=current_sum-Arr.get(i-K)+Arr.get(i);
